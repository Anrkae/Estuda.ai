...
@media (max-width: 600px) {
    ...
    #${popupId} .register-popup-drag-handle { gap: 4px; }
        ...
        // --- Event Listeners do Popup (Lógica de Arrastar sem alterações) ---
        if (popupContainer && popupHeader && popupDragHandle && formRegistroPopup) {
            ...
            let currentY = 0;
            let deltaY = 0;
            const dragThreshold = 70; // Mantido em 70

            const getClientY = (event) => { /* ... código getClientY ... */ if(event.touches&&event.touches.length>0){return event.touches[0].clientY}if(event.changedTouches&&event.changedTouches.length>0){return event.changedTouches[0].clientY}return event.clientY};

            const handleDragStart = (event) => { /* ... código handleDragStart ... */ if(!popupContainer.classList.contains('level-1'))return;isDragging=!0;startY=getClientY(event);deltaY=0;popupContainer.style.transition='none';document.addEventListener('mousemove',handleDragMove);document.addEventListener('touchmove',handleDragMove,{passive:!1});document.addEventListener('mouseup',handleDragEnd);document.addEventListener('touchend',handleDragEnd);document.addEventListener('mouseleave',handleDragEnd);if(event.type!=='touchstart'){event.preventDefault()}};

            const handleDragMove = (event) => { /* ... código handleDragMove ... */ if(!isDragging)return;event.preventDefault();currentY=getClientY(event);deltaY=currentY-startY;if(deltaY>0){popupContainer.style.transform=`translateY(${deltaY}px)`}else{popupContainer.style.transform='translateY(0px)'}};

            const handleDragEnd = (event) => { /* ... código handleDragEnd ... */ if(!isDragging)return;isDragging=!1;popupContainer.style.transition='';popupContainer.style.transform='';if(deltaY>dragThreshold){popupContainer.classList.remove('level-1');popupContainer.classList.add('level-2');if(feedbackDiv)feedbackDiv.style.display='none'}document.removeEventListener('mousemove',handleDragMove);document.removeEventListener('touchmove',handleDragMove);document.removeEventListener('mouseup',handleDragEnd);document.removeEventListener('touchend',handleDragEnd);document.removeEventListener('mouseleave',handleDragEnd)};
            ...
            console.error({popupContainer:!!popupContainer,popupHeader:!!popupHeader,popupDragHandle:!!popupDragHandle,formRegistroPopup:!!formRegistroPopup});
        }
    }

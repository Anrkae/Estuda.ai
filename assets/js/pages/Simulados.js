...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Delegação de eventos para botões dentro das questões
    questoesOutput.addEventListener('click', (event) => {
        const target = event.target;
        if (target.matches('.option-btn')) {
            // Verifica se o botão clicado não está desabilitado (segurança extra)
            if (!target.disabled) {
                handleOptionClick(target);
            }
        } else if (target.matches('.confirm-answer-btn')) {
             // Verifica se o botão clicado não está desabilitado
             if (!target.disabled) {
                 handleConfirmAnswer(target);
             }
        }
    });
    ...
    function clearOutput() {
     ...
     function saveSessionSummary() {
        if (!currentSessionStats.id || currentSessionStats.totalQuestions === 0) {
            console.log("Nenhuma sessão ativa ou válida para salvar.");
            return;
        }
        ...
        if (window.timerPopupAPI && typeof window.timerPopupAPI.getDuration === 'function') {
            try {
             ...
             console.warn("Função timerPopupAPI.getDuration() não encontrada. Usando cálculo manual.");
        }

        const summary = {
            id: currentSessionStats.id, timestamp: new Date().toISOString(),
            disciplina: currentSessionStats.disciplina, totalQuestions: currentSessionStats.totalQuestions,
            answeredCount: currentSessionStats.answeredCount, correctCount: currentSessionStats.correctCount,
        ...
        try {
            ...
            const index = existingSummaries.findIndex(s => s.id === summary.id);
             if (index === -1) {
                 ...
                 existingSummaries[index] = summary; // Atualiza se já existir
             }
            localStorage.setItem(RESULTS_STORAGE_KEY, JSON.stringify(existingSummaries));
            console.log(`Resumo da sessão ${summary.id} salvo com sucesso.`);
        } catch (error) {
    ...
    function finalizeSession(openPanel = false) {
        if (currentSessionStats.totalQuestions === 0 || !currentSessionStats.id) {
            console.log("Tentativa de finalizar sessão, mas nenhuma está ativa.");
            return;
        }
        const sessionId = currentSessionStats.id;
        console.log(`Finalizando sessão ID: ${sessionId}. Flag openPanel=${openPanel}`); // Log flag
        ...
        if (window.timerPopupAPI && typeof window.timerPopupAPI.stopTimer === 'function') {
            ...
            } catch (e) { console.error("Erro ao chamar stopTimer:", e); }
        } else { console.warn('Função timerPopupAPI.stopTimer() não encontrada.'); }

        saveSessionSummary();
        const wasActive = currentSessionStats.id; // Guarda se estava ativa antes de resetar
        resetSessionState(); // Reseta ANTES de abrir painel

        if (openPanel && wasActive) { // Abre painel apenas se solicitado E uma sessão foi de fato finalizada
            console.log("finalizeSession: Condition openPanel=true met. Attempting to open panel."); // Log
            if (window.timerPopupAPI && typeof window.timerPopupAPI.openPanel === 'function') {
                 ...
                 } catch (e) { console.error("Erro ao chamar openPanel:", e); }
            } else {
        ...
        } else if (wasActive) { // Só mostra status se uma sessão foi finalizada
             console.log("finalizeSession: Condition openPanel=false met. NOT opening panel."); // Log
    ...
    // (Sem alterações significativas aqui, a lógica anterior parecia ok)
        ...
        questionBlocks.forEach((block, index) => {
            try {
                ...
                lines.forEach(line => {
                    ...
                    else if (/^\[G\]/i.test(line)) { questionData.suggestedAnswer = line.substring(3).trim(); }
                });
                ...
                questions.push({
                    id: `q-error-${Date.now()}-${index}`, text: `Erro ao carregar esta questão (${error.message}). Verifique o console para detalhes.`,
    ...
    function displayParsedQuestions(questionsArray) {
        ...
        questionsArray.forEach((qData, index) => {
            ...
            if (qData.type === 'multipla_escolha' || qData.type === 'verdadeiro_falso') {
                 ...
                 questionDiv.dataset.selectedOption = ''; // Inicializa vazio
            }
    ...
    function handleOptionClick(clickedButton) {
        ...
        // Atualiza estado visual e lógico da seleção
        allOptionButtons.forEach(btn => btn.classList.remove('selected-preview'));
        clickedButton.classList.add('selected-preview');
        questionDiv.dataset.selectedOption = selectedValue; // GUARDA a seleção

        console.log(`Set selectedOption=${selectedValue} on ${questionDiv.id}`); // Log

        // Habilita o botão "Responder" correspondente
        if (confirmAnswerBtn) {
            confirmAnswerBtn.disabled = false;
            console.log(`Enabled confirm button for ${questionDiv.id}`); // Log
        } else {
             console.error("Botão .confirm-answer-btn não encontrado para", questionDiv.id);
        }
    }
    ...
    function handleConfirmAnswer(confirmButton) {
         console.log("handleConfirmAnswer triggered for button:", confirmButton); // Log
         const questionDiv = confirmButton.closest('.question-item');
         console.log("Associated questionDiv:", questionDiv); // Log

         if (!questionDiv) {
              console.error("Não foi possível encontrar o .question-item pai do botão.");
              return;
         }
         ...
         // Log detalhado das condições ANTES da verificação
         console.log("Checking conditions: ", {
             userAnswer: userAnswer, // Verifica se tem valor
             isAnswered: isAnswered,
             sessionId: currentSessionStats.id
         });

        // Verifica se tem uma opção selecionada E se a questão NÃO foi respondida
        if (!userAnswer || isAnswered) {
             console.warn("Tentativa de confirmar resposta inválida: Nenhuma opção selecionada ou questão já respondida.", {
                  selectedOption: userAnswer, // Loga o valor (pode ser string vazia)
                  isAnswered: isAnswered
              });
             // Adiciona um feedback visual rápido para o usuário
             confirmButton.style.opacity = '0.5';
             setTimeout(() => { confirmButton.style.opacity = '1'; }, 300);
             return;
        }
        // Verifica se a sessão está ativa
         if (!currentSessionStats.id) {
              console.warn("Tentativa de responder sem sessão ativa.");
              showStatus("Erro: Sessão não iniciada.", "error");
              return;
         }

        // --- Passou nas verificações ---
        console.log("Passed initial checks. Proceeding with answer evaluation...");
        ...
        // Marca como respondido (estado e UI)
        questionDiv.dataset.answered = 'true';
        questionDiv.classList.add('answered');
        questionDiv.classList.add(isCorrect ? 'correct' : 'incorrect');

        // Atualiza botões de opção
        allOptionButtons.forEach(btn => {
            btn.disabled = true;
            btn.classList.remove('selected-preview'); // Garante remoção da pré-seleção
             if (btn === originallySelectedButton) {
                 btn.classList.add('selected'); // Marca a selecionada final
             }
            if (btn.dataset.value === correctAnswer) {
                btn.classList.add('correct-answer-highlight'); // Destaca a correta
            }
        });

        // Desabilita o botão Responder
         confirmButton.disabled = true;
        ...
        if (feedbackDiv) {
            ...
            feedbackDiv.style.display = 'block';
             ...
             console.warn("Elemento .feedback-message não encontrado para", questionDiv.id);
        }
    ...
    async function handleGenerateQuestions() {
        ...
        if (!assunto) {
             assuntoInput.focus(); return showError("Por favor, informe o Assunto Principal.");
        }
        if (isNaN(numQuestoes) || numQuestoes < 1 || numQuestoes > 20) {
             numQuestoesInput.focus(); return showError("Número de questões inválido (1-20).");
        }
        if (!GEMINI_API_KEY || !GEMINI_API_KEY.startsWith('AIza') || GEMINI_API_KEY.length < 30) {
             return showError("Erro Crítico: Chave da API Gemini inválida ou ausente.");
        }

        const disciplinaParaSessao = disciplinaSelecionada || "Geral";
        console.log(`Iniciando geração. Assunto: "${assunto}". Biblio: "${bibliografia}". Disciplina: "${disciplinaParaSessao}". Tipo: ${tipoQuestao}. Número: ${numQuestoes}`);
        showLoading(true);
        clearOutput();
        ...
        switch (tipoQuestao) {
            case 'multipla_escolha':
                prompt += `  - Para CADA alternativa, use EXATAMENTE o formato "[A] texto da alternativa A", "[B] texto da B", etc. (Use A, B, C, D).\n`;
                prompt += `  - Indique a resposta correta usando "[R] " seguido APENAS pela LETRA maiúscula correta (A, B, C ou D). Ex: "[R] C".\n`;
                prompt += `Exemplo Múltipla Escolha:\n[Q] Qual a capital da França?\n[A] Londres\n[B] Berlim\n[C] Paris\n[D] Madri\n[R] C\n[SEP]`;
                break;
            case 'verdadeiro_falso':
                prompt += `  - Forneça a afirmação no enunciado [Q].\n`;
                prompt += `  - Use "[V]" (pode adicionar justificativa curta após, ex: "[V] Sim, está correto pois...") ou deixe vazio.\n`;
                prompt += `  - Use "[F]" (pode adicionar justificativa curta após, ex: "[F] Não, o correto seria...") ou deixe vazio.\n`;
                prompt += `  - Indique a resposta correta usando "[R] " seguido APENAS por "V" ou "F". Ex: "[R] V".\n`;
                prompt += `Exemplo Verdadeiro/Falso:\n[Q] A Terra é plana?\n[V]\n[F] A Terra é um geoide.\n[R] F\n[SEP]`;
                break;
            case 'dissertativa_curta':
                prompt += `  - Forneça uma resposta/gabarito curto e direto usando "[G] ". Ex: "[G] Pedro Álvares Cabral.".\n`;
        ...
        try {
            const requestBody = { contents: [{ parts: [{ text: prompt }] }],
                 generationConfig: { "temperature": 0.7, "maxOutputTokens": 350 * numQuestoes + 200 },
                 safetySettings: [
            ...
            const response = await fetch(API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(requestBody) });
             ...
             if (!response.ok) {
                 let errorBodyText = await response.text(); console.error("Raw API Error Response:", errorBodyText);
                 let errorBody = {}; try { errorBody = JSON.parse(errorBodyText); } catch (e) { /* Ignora */ }
                 const detailMessage = errorBody?.error?.message || `Erro HTTP ${response.status}`;
                 if (detailMessage.includes("API key not valid")) { throw new Error("Falha na API Gemini: A Chave da API configurada não é válida."); }
                 else { throw new Error(`Falha na comunicação com a API Gemini: ${detailMessage}`); }
             }
            ...
            if (data.promptFeedback?.blockReason) {
                  ...
                  if(data.promptFeedback.safetyRatings?.length > 0) { const blockedCategory = data.promptFeedback.safetyRatings.find(r => r.blocked)?.category; if (blockedCategory) blockDetails += ` - Categoria: ${blockedCategory}`; }
                  showError(`A solicitação foi bloqueada por filtros de segurança (${blockDetails}). Tente reformular.`); return;
             }
            if (!data.candidates || data.candidates.length === 0 || !data.candidates[0].content?.parts?.length > 0 || !data.candidates[0].content.parts[0].text) {
                 console.error("Resposta inesperada da API (sem candidatos/conteúdo válido):", data);
                 showError("Erro: A API retornou uma resposta vazia ou em formato inesperado."); return;
            }
            ...
            if (totalValidQuestions > 0) {
                ...
                if (window.timerPopupAPI && typeof window.timerPopupAPI.startSession === 'function') {
                     try {
                         console.log(`Iniciando sessão no Timer Popup ID: ${currentSessionStats.id}`);
                         window.timerPopupAPI.startSession( currentSessionStats.totalQuestions, currentSessionStats.disciplina );
                         console.log("handleGenerateQuestions SUCCESS: Called startSession. Explicitly NOT calling openPanel here."); // Log Confirmação Req 4
                     } catch (e) { console.error("Erro ao chamar startSession:", e); }
                     finalizeButton.style.display = 'inline-flex';
                     const successMsg = totalValidQuestions === numQuestoes ? `Geradas ${totalValidQuestions} questões! Sessão iniciada.` : `Geradas ${totalValidQuestions} de ${numQuestoes} solicitadas. Sessão iniciada.`;
                     showStatus(successMsg, 'success');
                } else {
                    console.warn('API do Timer Popup (startSession) não encontrada.');
                     finalizeButton.style.display = 'inline-flex';
                    showStatus('Questões geradas, mas o timer externo não pôde ser iniciado.', 'warning');
                }
            } else {
                if (questionsArray.length > 0) { showError(`Erro: ${questionsArray.length} questão(ões) retornada(s) pela API, mas nenhuma pôde ser processada corretamente. Verifique o console.`); }
                else { showError("Erro: Nenhuma questão foi retornada pela API ou o formato estava totalmente irreconhecível."); }
                resetSessionState();
            }
